...
from reading_plans import READING_PLANS, get_plan, plan_summary
...
def security_headers(response):
    ...
    if response.mimetype == "text/html":
        ...
        response.headers.setdefault("Content-Security-Policy", "; ".join([
            ...
            f"script-src 'self' 'unsafe-inline' {NATIVE_ORIGINS}",
...
# Endpoints that legitimately accept POSTs without a browser form token.
# Push subscription and sync are called by our own JavaScript, which sends the
# token in a header instead; they are listed here only so the check knows to
# look at the header rather than the form body.
CSRF_HEADER_ENDPOINTS = {
    "push_subscribe", "push_unsubscribe", "prayer_pray",
    "sync_get", "sync_put", "sync_bulk", "study_ask",
    "submit_game_score", "poll_vote",
}
...
def protect_against_csrf():
    ...
    if endpoint in CSRF_HEADER_ENDPOINTS:
    ...
    if not expected or not supplied or not secrets.compare_digest(
        ...
        if request.is_json or endpoint in CSRF_HEADER_ENDPOINTS:
...
def account_delete():
    ...
    db = get_db()
    row = db.execute(
        "SELECT password_hash FROM members WHERE id=?", (m["id"],)
    ).fetchone()

    if not row or not row["password_hash"] or not verify_password(
        row["password_hash"], password
    ):
        flash("That password wasn't right, so nothing was deleted.")
        return redirect(url_for("account"))
...
def sync_put(kind):
    ...
    db.commit()
    return {"ok": True, "signed_in": True}, 200
...
def fetch_chapter(book_name, chapter, translation):
    query = f"{book_name} {chapter}".replace(" ", "+")
    resp = requests.get(
        f"https://bible-api.com/{query}", params={"translation": translation}, timeout=6
    )
...
def submit_game_score(game_key):
    ...
    return {"ok": True, "signed_in": True, "improved": improved, "best": best}, 200
...
@app.route("/games")
def games_hub():
    return render_template("games.html", church_name=CHURCH_NAME, games=GAMES)
